...
{    
    ...
    uint32_t itr = 0;
    float field = 0., T1=0., T2=0., rf_phase = param->RF_PH_deg.ptr[0], time_elapsed = 0.; 
    float m0[3], m1[3]; 
    ...
    double diffusivity_scale = param->diffusivity.ptr[ts_old];
    ...
    for (uint32_t dummy_scan = 0; dummy_scan < param->n_dummy_scan + 1; dummy_scan++)
    {
        ...
        while (current_timepoint < param->n_timepoints) // param->n_timepoints is the total number of timepoints (= TR/dwelltime)
        {
            ...
            {   
                ...
                if (ts != ts_old) 
                {
                    if (dist_cross_tissue(gen_u) >= param->pXY.ptr[ts_old*param->n_substrate + ts])
                    {
                ...
                T1 = param->T1_ms.ptr[ts_old] * 1e-3; // ms -> s
                T2 = param->T2_ms.ptr[ts_old] * 1e-3; // ms -> s
                diffusivity_scale = param->diffusivity.ptr[ts_old];
            }   
            accumulated_phase += field;
            itr = 0;

            // ------ apply ideal dephasing if there is any ------
            if(counter_dephasing < param->dephasing_us.size && param->dephasing_us.ptr[counter_dephasing] == current_timepoint)
            {
                accumulated_phase += (float)spin_no * param->dephasing_deg.ptr[counter_dephasing] / (float)param->n_spins; // assign dephasing linearly to spins 
                counter_dephasing++;
            }

            // ------ apply gradient if there is any ------
            if(counter_gradient < param->gradient_us.size && param->gradient_us.ptr[counter_gradient] == current_timepoint)
            {
                const float *Gxyz = param->gradient_mTm.ptr + 3*counter_gradient;
            ...
            if(current_rf < param->RF_us.size && param->RF_us.ptr[current_rf] == current_timepoint)
            {
                // dephase and relax    
                ...
                // apply RF pulse
                xrot_withphase (param->RF_FA_deg.ptr[current_rf], param->RF_PH_deg.ptr[current_rf], m1, m0); // Note m0 and m1 are swapped here, so that we can use m0 for the next iteration
            ...
            if (is_lastscan && current_te < param->TE_us.size && param->TE_us.ptr[current_te] == current_timepoint)
            {
                // dephase and relax                
                time_elapsed = (current_timepoint - old_timepoint) * param->timestep_us * 1e-6;
                dephase_relax(m0, m1, accumulated_phase, T1, T2, time_elapsed);
                // save echo and copy m1 to m0 for the next iteration
                for (uint32_t i=0, shift=3*param->TE_us.size*spin_no + 3*current_te; i<3; i++)
                    M1[shift + i] = m0[i] = m1[i];
                
                T[spin_no*param->TE_us.size + current_te] = ts_old;
