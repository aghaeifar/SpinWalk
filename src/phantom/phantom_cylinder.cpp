...
{   
    ...
    for (c = 0; c < m_cylinder_radii.size(); c++)
    {
        ...
        if (m_calc_fieldmap)
        {
            x_min = std::max(0, cyl_pnt_vox[0] - cyl_rad_vox*20);
            x_max = std::min((int32_t)m_resolution, cyl_pnt_vox[0] + cyl_rad_vox*20);
            y_min = std::max(0, cyl_pnt_vox[1] - cyl_rad_vox*20);
            y_max = std::min((int32_t)m_resolution, cyl_pnt_vox[1] + cyl_rad_vox*20);
        } else 
        {
